...
def get_FLAGS():
    ...
    parser.add_argument(
      ...
      type=str,
      required=False,
      default="/home/cooper/data-ssd/data5/cxm/ljx/BEV-MF/prediction_save_dir_KITTI/80.22-83.11-0.266/",
      help=''
  )
    ...
    parser.add_argument(
      ...
      type=str,
      required=False,
      default="/home/cooper/data-ssd/data5/cxm/MOS/BEV-MF-PR/prediction_save_dir_KITTI/",
      help=''
  )
    parser.add_argument(
      '--movable_label', '-ma',
...
def get_iou(label_names,pred_label_names,n_classes,learning_map,label_str = None):
    conf_matrix = np.zeros((n_classes,n_classes),dtype=np.int64)
    iou_list = []
    with tqdm.tqdm(total=len(label_names),ncols=80) as pbar:  # total为进度条总的迭代次数
...
def main():
    ...
    for sequence in sequences:
        ...
        if label_path!= None:
            label_paths = os.path.join(label_path, "sequences", str(sequence), "predictions")
        ...
        if movable_label_path!= None:
            movable_label_paths = os.path.join(movable_label_path, "sequences", str(sequence), "labels")
            # populate the label names
    ...
    if moving_label_path!= None:
        moving_iou,moving_iou_list = get_iou(label_names[:], moving_names[:],moving_n_classes,moving_learning_map,moving_label_str) 
        moving_iou_list=np.array(moving_iou_list,dtype=np.float64)
        moving_save_dir = f'./iou_list/moving_iou_list.npy'
        ...
        for class_name, class_iou in zip(moving_label_str, moving_iou):
            print('%s : %.2f%%' % (class_name, class_iou * 100))
    if movable_label_path!= None:
        movable_iou,movable_iou_list = get_iou(label_names[:], movable_names[:],movable_n_classes,movable_learning_map,movable_label_str) 
