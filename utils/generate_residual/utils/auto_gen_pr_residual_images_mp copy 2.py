...
from kitti_utils import load_poses, load_calib, load_files, load_vertex , polar_projection, polar_projection2 , range_projection , load_yaml , check_and_makedirs
    ...
    for frame_idx in tqdm(range(len(scan_paths)),desc=f"{seq}"):
        ...
        for rv_last_n in range(1, rv_num_last_n+1):
            ...
            if frame_idx < rv_last_n:
                ...
                rv_current_idx, rv_current_range = range_projection(current_scan.astype(np.float32),
                ...
                rv_last_idx_transformed, rv_last_range_transformed = range_projection(last_scan_transformed.astype(np.float32),
                                                          range_image_params['height'], range_image_params['width'],
                                                          range_image_params['fov_up'], range_image_params['fov_down'],
                                                          range_image_params['max_range'], range_image_params['min_range'])
        ...
        if frame_idx < polar_num_prev_n + polar_num_last_n - 1:
            ...
            if frame_idx == polar_num_prev_n + polar_num_last_n - 1:  # initialize                   idx = 7时，初始化设置
                for i in range(-polar_num_last_n - polar_num_prev_n + 1, -polar_num_last_n + 1):   #处理-7到-4帧
                    ...
                    if prev_sub_map is None:
                        ...
                        prev_index = np.concatenate((prev_index, np.full(prev_scan.shape[0], polar_num_last_n, dtype=int)),    # prev_idx按照4，5，6，7排序，代表后续存放通道
                                                    axis=0)
                    # print("prev_scan.shape[0]",prev_scan.shape[0])
                        # print("prev_scan.shape[0]",prev_scan.shape[0])
                ...
                for i in range(-polar_num_last_n + 1, 1):    #处理-3到0帧
                    ...
                    # print("last_scan.shape[0]",last_scan.shape[0])
            else:
            ...
            # print("prev_diff_map",prev_diff_map.shape)
        ...
        ####### polar end #######
            ...
            scan = load_vertex(scan_paths[frame_idx - polar_num_last_n - polar_num_prev_n + 1])
            _, _, _, _, _,y,x,bev_range = \
                polar_projection2(current_vertex=scan.astype(np.float32),
                                 proj_H=polar_image_params['height'],
                                 proj_W=polar_image_params['width'],
                                 max_range=polar_image_params['max_range'],
                                 min_range=polar_image_params['min_range'],
                                 ...
                                 min_z=polar_image_params['min_z'],
                                 return_occlusion=polar_occlusion_block)
            # print("prev_proj_y",len(prev_proj_y))
            # print("prev_proj_x",len(prev_proj_x))
            # print("prev_diff_map111",prev_diff_map.shape)
            # print("diff_scan",diff_scan.shape)
            # print(concatenated_data.shape)
            # np.save(polar_file_name, concatenated_data)
            
            for channel in range(8):
                bev_range[y,x] = diff_scan[:,channel]
                # print(np.max(bev_range),np.min(bev_range),np.mean(bev_range))
                bev_range = (bev_range - np.min(bev_range))/(np.max(bev_range)-np.min(bev_range)+1e-6)*2-1
                # bev_range[bev_range<=0.01] = 0
                bev_visualize = True
                if bev_visualize:
                        fig = plt.figure(frameon=False, figsize=(16, 10))
                        fig.set_size_inches(20.48, 0.64)
                        ...
                        ax.set_axis_off()
                        fig.add_axes(ax)
                        ax.imshow(bev_range, vmin=0, vmax=1)
                        bev_visualization_folder = os.path.join(residual_image_folder.replace("residual_images","bev_visualization-3"))
                        check_and_makedirs(bev_visualization_folder)
                        image_name = os.path.join(bev_visualization_folder,str(frame_idx - polar_num_last_n - polar_num_prev_n + 1).zfill(6))
                        image_name = image_name + "-"+str(channel)
                        plt.savefig(image_name)
                        plt.close()
...
if __name__ == '__main__':
    ...
    # for SemanticKITTI
    for seq in range(3, 4):  # sequences id
