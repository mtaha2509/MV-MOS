...
from kitti_utils import load_poses, load_calib, load_files, load_vertex , polar_projection , range_projection , load_yaml , check_and_makedirs
...
def gen_pr_residual_images(cfg,seq):
    ...
    for frame_idx in tqdm(range(len(scan_paths)),desc=f"{seq}"):
        ...
        if frame_idx < polar_num_prev_n + polar_num_last_n - 1:
            current_scan = load_vertex(scan_paths[frame_idx])
            diff_scan = np.full((current_scan.shape[0], rv_num_last_n), 0, dtype=np.float32)
...
if __name__ == '__main__':
    # load config file
    config_filename = './pr_residual_images.yaml'
