...
from kitti_utils import load_poses, load_calib, load_files, load_vertex , polar_projection , range_projection , load_yaml , check_and_makedirs, range_projection3
    ...
    for frame_idx in tqdm(range(len(scan_paths)),desc=f"{seq}"):
        ...
        for rv_last_n in range(1, rv_num_last_n+1):
            ...
            if frame_idx < rv_last_n:
                ...
                proj_mask, proj_range,proj_x_orig,proj_y_orig = range_projection3(current_scan.astype(np.float32),
                ...
                rv_mask_array.append(proj_mask)
                rv_range_array.append(proj_range)
                rv_x_array.append(proj_x_orig)
                rv_y_array.append(proj_y_orig)
                # print("rv_current_scan",current_scan.shape)
                print("proj_x_orig",len(proj_x_orig))
            else:
        ...
        ####### polar end #######
            ...
            print("bev_diff_scan",diff_scan.shape)
            p2r = rv_range_array.pop(0)
            p2r_x = rv_x_array.pop(0)
            p2r_y = rv_y_array.pop(0)
            p2r_mask = rv_mask_array.pop(0)
            p2r[p2r_y,p2r_x] = diff_scan[p2r_mask][:,0]
            bev_visualize = True
            if bev_visualize:
                    fig = plt.figure(frameon=False, figsize=(16, 10))
                    fig.set_size_inches(20.48, 0.64)
                    ...
                    ax.set_axis_off()
                    fig.add_axes(ax)
                    ax.imshow(p2r, vmin=0, vmax=1)
                    bev_visualization_folder = rv_visualization_folder.replace("visualization","bev_visualization")
                    image_name = os.path.join(bev_visualization_folder, str(rv_last_n))
                    check_and_makedirs(image_name)
                    image_name = os.path.join(image_name,str(frame_idx).zfill(6))
                    plt.savefig(image_name)
                    plt.close()
...
if __name__ == '__main__':
    ...
    # for SemanticKITTI
    for seq in range(0, 1):  # sequences id
