...
if __name__ == '__main__':
    ...
    # load config file
    config_filename = './data_preparing_polar_sequential.yaml'
    config = load_yaml(config_filename)

    scan_folder = config['scan_folder']
    scan_folder_test = config['scan_folder_test']
    ...
    for seq in range(11, 22):  # sequences id
        # Update the value in config to facilitate the iterative loop
        config['scan_folder'] = scan_folder_test + f"sequences/{'%02d' % seq}/velodyne"
        config['pose_file'] = scan_folder_test + f"sequences/{'%02d' % seq}/poses.txt"
        config['calib_file'] = scan_folder_test + f"sequences/{'%02d' % seq}/calib.txt"
        config['residual_image_folder'] = residual_image_folder + f"{'%02d' % seq}/residual_images"
        # ic(config)
        # process_one_seq(config)
        process = Process(target=process_one_seq, args=(config, seq))  # 指定进程要执行的函数和参数
