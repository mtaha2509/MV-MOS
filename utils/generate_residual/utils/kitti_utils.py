...
def range_projection2(current_vertex, proj_H=64, proj_W=2048, fov_up=3.0, fov_down=-25.0, max_range=50, min_range=2):
    """ Project a pointcloud into a spherical projection, range image.
        Args:
            current_vertex: raw point clouds
        Returns:
    ...
    # mask = (depth_ori > min_range) & (depth_ori < max_range)    
    scan_z = current_vertex[:, 2]
    mask = (depth_ori > 1.85) & (depth_ori < 50.15) & (scan_z < 2) & (scan_z > -4)
    current_vertex = current_vertex[mask]  # get rid of [0, 0, 0] points
    depth = depth_ori[mask]
    ...
    return mask, proj_range,proj_y_orig, proj_x_orig,proj_idx

def range_projection3(current_vertex, proj_H=64, proj_W=2048, fov_up=3.0, fov_down=-25.0, max_range=50, min_range=2):
    """ Project a pointcloud into a spherical projection, range image.
        Args:
            current_vertex: raw point clouds
        Returns:
    ...
    scan_z = current_vertex[:, 2]
    mask = (rho > 1.85) & (rho < 50.15) & (scan_z < 2) & (scan_z > -4)
...
def polar_projection2(current_vertex, proj_H=360, proj_W=480, max_range=50.0, min_range=2.0, max_z=2.0, min_z=-4.0,
                     return_occlusion=False):
    ### for rv ###
    rv_height=64
    rv_width=2048
    ...
    rv_min_range= 1.85
    # laser parameters
    rv_fov_up = rv_fov_up / 180.0 * np.pi  # field of view up in radians
    rv_fov_down = rv_fov_down / 180.0 * np.pi  # field of view down in radians
    rv_fov = abs(rv_fov_down) + abs(rv_fov_up)  # get field of view total in radians
    # get depth of all points
    rv_depth = np.linalg.norm(current_vertex[:, :3], 2, axis=1)
    # get scan components
    rv_scan_x = current_vertex[:, 0]
    rv_scan_y = current_vertex[:, 1]
    rv_scan_z = current_vertex[:, 2]
    # get angles of all points
    rv_yaw = -np.arctan2(rv_scan_y, rv_scan_x)
    rv_pitch = np.arcsin(rv_scan_z / rv_depth)
    # get projections in image coords
    rv_proj_x = 0.5 * (rv_yaw / np.pi + 1.0)  # in [0.0, 1.0]
    rv_proj_y = 1.0 - (rv_pitch + abs(rv_fov_down)) / rv_fov  # in [0.0, 1.0]

    # scale to image size using angular resolution
    rv_proj_x *= rv_width  # in [0.0, W]
    rv_proj_y *= rv_height  # in [0.0, H]
        # round and clamp for use as index
    rv_proj_x = np.floor(rv_proj_x)  # 向下取整
    rv_proj_x = np.minimum(rv_width - 1, rv_proj_x)
    rv_proj_x = np.maximum(0, rv_proj_x).astype(np.int32)  # in [0,W-1]

    rv_proj_y = np.floor(rv_proj_y)
    rv_proj_y = np.minimum(rv_height - 1, rv_proj_y)
    rv_proj_y = np.maximum(0, rv_proj_y).astype(np.int32)  # in [0,H-1]
    rv_proj_range = np.full((rv_height, rv_width), 0, dtype=np.float32)  # [H,W] range (-1 is no data)
    ### for rv ###
    ...
    yaw_valid = yaw[mask_valid]
    z_valid = scan_z[mask_valid]
    # rv_proj_x = rv_proj_x[mask_valid]
    # rv_proj_y = rv_proj_y[mask_valid]
    ...
    # print("proj_z_delta",proj_z_delta.shape)
    # print("mask_valid",len(mask_valid))
    # print("proj_y",len(proj_y))

    # print("rv_proj_y",len(rv_proj_y))
    # print("rv_proj_x",len(rv_proj_x))
    # print("rv_proj_range",rv_proj_range.shape)
    return None, None, None, None, None,rv_proj_y,rv_proj_x,rv_proj_range
