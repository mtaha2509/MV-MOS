...
def main(arch_config, data_config):
    ...
    if not os.path.exists(model_load_path):
        raise ValueError("model load path doesn't exist, please check the infer.yaml")
    ...
    # dynamically loading start #
    time_name = model_load_path.split('/')[-3] # get the time name path
    # ic(time_name)
    code_back_up_path = os.path.join("model_save_dir",time_name,"code_backup")
    ic(code_back_up_path)
    if not os.path.exists(code_back_up_path):
        raise ValueError(f"code back up path doesn't exist, please check model_save_dir if there is \"{code_back_up_path}\" ")
    # network_path = os.path.join(code_back_up_path,"network")
    # # from network.CA_BEV_Unet import CA_Unet
    # CA_BEV_Unet_Module = imp.load_source("CA_BEV_Unet_Module",f"{network_path}/CA_BEV_Unet.py")
    # CA_Unet = CA_BEV_Unet_Module.CA_Unet
    # # from network.ptBEVnet import ptBEVnet
    # ptBEVnet_Module = imp.load_source("ptBEVnet_Module",f"{code_back_up_path}/network/ptBEVnet.py")
    # ptBEVnet = ptBEVnet_Module.ptBEVnet
    # from network.PR_BEV_Unet import CA_Unet
    # from network.ptBEVnet import ptBEVnet
    # from network.PR_ptBEVnet import PR_net
    PR_BEV_Unet_Module = imp.load_source("PR_BEV_Unet_Module",f"{code_back_up_path}/network/PR_BEV_Unet.py")
    BEV_model = PR_BEV_Unet_Module.CA_Unet
    PR_ptBEVnet_Module = imp.load_source("PR_ptBEVnet_Module",f"{code_back_up_path}/network/PR_ptBEVnet.py")
    model = PR_ptBEVnet_Module.PR_net

    dataloader_path = os.path.join(code_back_up_path,"dataloader")
    # from dataloader.dataset import  collate_fn_BEV_MF_test,SemKITTI, get_SemKITTI_label_name_MF, spherical_dataset
    dataset = imp.load_source("ptBEVnet",f"{dataloader_path}/dataset.py")
    collate_fn_BEV_MF_test = dataset.collate_fn_BEV_MF_test
    SemKITTI = dataset.SemKITTI
    get_SemKITTI_label_name_MF = dataset.get_SemKITTI_label_name_MF
    spherical_dataset = dataset.spherical_dataset
    # dynamically loading end #
    ...
    # prepare model
    # model
    my_BEV_model = BEV_model(moving_n_class=len(moving_label),
    ...
    if val:
        ...
        with torch.no_grad():
            with tqdm(total=len(val_dataset_loader),desc='infer') as pbar_val:
                # for _, (_,_, val_grid, val_pt_labs_moving, _,val_pt_fea,val_index) in enumerate(val_dataset_loader):
                for i_iter_val, (val_moving_label,val_movable_label, val_grid, val_pt_labs_moving, val_pt_labs_movable,val_pt_fea, val_r2p_matrix, val_p2r_matrix, val_range_res,val_index) in enumerate(val_dataset_loader):    
                    ...
                    for i in range(len(val_r2p_matrix)):
                        val_r2p_matrix[i] = val_r2p_matrix[i].to(pytorch_device)
                        val_p2r_matrix[i] = val_p2r_matrix[i].to(pytorch_device)
    ...
    if test:
        ...
        with torch.no_grad():
            with tqdm(total=len(test_dataset_loader),desc='infer') as pbar_test:
                for _, (_,_, test_grid, _, _,test_pt_fea, test_r2p_matrix, test_p2r_matrix, test_range_res,test_index) in enumerate(test_dataset_loader): 
                    # predict
                    test_pt_fea_ten = [i.to(pytorch_device) for i in test_pt_fea]
                    test_grid_ten = [i.to(pytorch_device) for i in test_grid]
                    for i in range(len(test_r2p_matrix)):
                        test_r2p_matrix[i] = test_r2p_matrix[i].to(pytorch_device)
                        test_p2r_matrix[i] = test_p2r_matrix[i].to(pytorch_device)
                    moving_out, movable_out = my_model(test_pt_fea_ten, test_grid_ten,test_r2p_matrix,test_p2r_matrix,test_range_res, pytorch_device)
                    moving_predict_labels = torch.argmax(moving_out, dim=1)
                    moving_predict_labels = moving_predict_labels.cpu().detach().numpy()
