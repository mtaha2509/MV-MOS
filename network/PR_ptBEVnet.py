...
class PR_net(nn.Module):
    ...
    def forward(self, pt_fea, xyz_ind, r2p_matrix, p2r_matrix, range_res, cur_dev):
        # concate everything
        cat_pt_ind = []
        ...
        # 使用 torch.stack 在新的维度上拼接
        cat_r2p_matrix = torch.stack(r2p_matrix, dim=0)
        cat_p2r_matrix = torch.stack(p2r_matrix, dim=0)

        cat_r2p_matrix = cat_r2p_matrix.cuda(non_blocking=True)
        cat_p2r_matrix = cat_p2r_matrix[:, :, :, :2].cuda(non_blocking=True)

        cat_range_res =  torch.stack(range_res, dim=0)
        cat_range_res = cat_range_res.permute(0, 3, 1, 2).to(cur_dev)
        ...
        #     out_data, res_data)
        moving_output,movable_output = self.BEV_model(
            out_data, res_data, cat_r2p_matrix, cat_p2r_matrix,cat_range_res)
