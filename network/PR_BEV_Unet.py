...
from network.BaseBlocks import UpBlock, add_down, group_aggregation_bridge
...
class CA_Unet(nn.Module):
    ...
    def __init__(self, moving_n_class,movable_n_class, n_height, residual, dilation=1, group_conv=False, input_batch_norm=False, dropout=0.,
        ...
        self.movable_n_class = movable_n_class
        ...
        self.network =  UNet_8_pr_VSS4(moving_n_class * n_height,movable_n_class * n_height, n_height, residual, dilation, group_conv, input_batch_norm, dropout,
    ...
    def forward(self, x, res, r2p_matrix, p2r_matrix, range_res):
        moving,movable = self.network(x, res, r2p_matrix, p2r_matrix, range_res)
...
class UNet_3(nn.Module):
    ...
    def __init__(self, moving_n_class, movable_n_class,n_height, residual, dilation, group_conv, input_batch_norm, dropout, circular_padding,
        ...
        self.GAB4 = group_aggregation_bridge(256,256)
        self.GAB3 = group_aggregation_bridge(128,128)
        self.GAB2 = group_aggregation_bridge(64,64)
        self.GAB1 = group_aggregation_bridge(32,32)

        self.movable_GAB3 = group_aggregation_bridge(128,128)
        self.movable_GAB2 = group_aggregation_bridge(64,64)
        self.movable_GAB1 = group_aggregation_bridge(32,32)
        #dropblock = False
        if not using_UpBlock:
            self.up3 = up(512, 128, circular_padding, group_conv=group_conv, use_dropblock=dropblock, drop_p=dropout,PixelShuffle=PixelShuffle)
            self.up2 = up(256, 64, circular_padding, group_conv=group_conv, use_dropblock=dropblock, drop_p=dropout,PixelShuffle=PixelShuffle)
    ...
    def forward(self, x, res):
        ...
        movable_out = self.movable_up3(c4_appearance,c3_appearance)
        movable_out = self.movable_GAB3(movable_out,c2_appearance)

        movable_out = self.movable_up2(movable_out,c2_appearance)
        movable_out = self.movable_GAB2(movable_out,c1_appearance)

        movable_out = self.movable_up1(movable_out,c1_appearance)
        movable_out = self.movable_GAB1(movable_out,c0_appearance)
