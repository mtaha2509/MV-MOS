...
class Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        ...
        # self.cv1 = double_conv_circular(c1, c_, False)
        # self.cv2 = double_conv_circular(c_, c2, False)
...
class C2f_circular(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = double_conv_circular(c1, 2 * self.c, False)
        self.cv2 = double_conv_circular((2 + n) * self.c, c2, False)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class Block_VSS(nn.Module):
    '''expand + depthwise + pointwise'''

    def __init__(self, kernel_size, in_size, expand_size, out_size, nolinear, semodule, stride, n):
        super().__init__()
        self.stride = stride
        self.se = nn.Sequential(*(semodule for _ in range(n)))
...
if __name__ == '__main__':
    ...
    # model = C3_LVMB(64,64,1,True).cuda()
    model = Block_VSS(3,64,256,64,hswish(),VSSBlock(64),1,2).cuda()
    pred = model(inputs)
    print(pred.size())